...
.vector-chunk{ font-family:monospace; font-size:0.97em; background:#f1f6fa; border-left:4px solid #bcdffc; border-radius:4px; margin:12px 0 4px 0; padding:8px 14px;}
...
with st.sidebar:
    ...
    top_k = st.number_input("Max sources per answer", min_value=3, max_value=12, value=int(st.session_state.get("top_k", 7)), step=1, key="top_k")
...
def get_recent_chat_history_str(max_turns=3):
    # Builds a partial transcript from the last N (user+LLM) turns
...
def rag_llm(query, context_chunks, sources, custom_prompt, model, stream_cb, _temperature, _top_p, _max_tokens, _repeat_penalty):
    ...
    chat_history_str = get_recent_chat_history_str(max_turns=3)  # most recent 3 turns (u/a/u/a)
...
def show_chunks(chunks, vec_hits):
    ...
    for i, c in enumerate(chunks, 1):
        st.markdown(
            f"<div class='vector-chunk'><b>[{i}]</b> <span style='color:#7a76c2;font-size:0.99em;'>"
            f"{c.get('source','(unknown)')}, score={c.get('score',0):.4f}</span><br>"
            f"{c.get('text')[:320]}{'...' if len(c.get('text',''))>320 else ''}</div>",
            unsafe_allow_html=True
        )
    if vec_hits:
        st.caption("Semantic (vector-only) top results for reference:")
        for i,v in enumerate(vec_hits,1):
            st.markdown(
                f"<div class='vector-chunk'><b>Vec#[{i}]</b> <span style='font-size:0.96em;color:#547ad7;'>"
                f"{v.get('source','(unknown)')}, score={v.get('score',0):.4f}</span><br>"
                f"{v.get('text','')[:200]}{'...' if len(v.get('text',''))>200 else ''}</div>",
                unsafe_allow_html=True
            )
...
with st.container():
    ...
    if user_prompt:
        ...
        doc_hits, vec_hits = hybrid_search(user_prompt, top=int(top_k))

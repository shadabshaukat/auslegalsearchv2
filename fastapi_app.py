"""
FastAPI backend for AUSLegalSearchv2, now with guaranteed chat_history support for RAG only if supported by RAGPipeline.query signature.
Only valid arguments will ever be passed, so no TypeError from chat_history.
"""
...
def download_hf_model(hf_repo):
    try:
        ...
        CrossEncoder(hf_repo)  # Triggers local download/cache
    except Exception as e:
        print(f"Could not download {hf_repo}: {e}")

app = FastAPI(
    title="AUSLegalSearchv2 API",
    description="REST API for legal vector search, ingestion, RAG, chat, and reranker model management.",
    version="0.20"
)
...
def api_search_rerank(req: SearchReq, _: str = Depends(get_current_user)):
    ...
    # For demo: sort by vector score; production: cross-encoder scoring.
...
# --- RAG/Llama QA with conversation memory ---
...
def api_search_rag(req: RagReq, _: str = Depends(get_current_user)):
    ...
    # Prepare arguments, only pass those supported by RAGPipeline.query()
    query_kwargs = dict(
    ...
    # Dynamically pass chat_history ONLY if query supports it.
